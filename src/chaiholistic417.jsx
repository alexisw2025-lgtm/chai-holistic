...
function LinkUrlInput({ onCommit }) {
  ...
  return (
    <div>
      ...
      {showTest && (
        <button
          onClick={()=>{ window.open(val.trim(),'_blank','noopener,noreferrer'); onCommit(val.trim()); }}
...
You may recommend up to 2 blends per response. Only use blend IDs from the catalog above. Do not include the tag mid-sentence — place it on its own line after your full response.`;
  ...
  const LargeTimerOverlay = () => {
    ...
    <div className="timer-overlay">
      ...
      <div className="timer-overlay-btns">
        ...
        <button
          onClick={() => { dismissTimer(); window.open("https://2amcompanion.com","_blank","noopener,noreferrer"); }}
  ...
  const Home = () => {
    ...
    <div>
      ...
      {/* ── START HERE — guided entry for new visitors ─────────────────────── */}
      {(()=>{
        const START_PATHS = [
        ...
        return (
          <section style={{background:"linear-gradient(180deg,#0A0F0B 0%,#0E1610 60%,#0A0F0B 100%)",padding:"64px 20px 72px",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{maxWidth:560,margin:"0 auto",position:"relative"}}>
              <div style={{textAlign:"center",marginBottom:36}}>
                ...
                <p style={{fontFamily:"Jost,sans-serif",fontSize:"clamp(13px,3.5vw,15px)",color:"rgba(247,242,234,0.5)",lineHeight:1.7,fontWeight:300,maxWidth:400,margin:"0 auto"}}>Chai Holistic is a living ecosystem — teas, prayers, rings, and rituals rooted in ancestral wisdom. Choose what calls to you.</p>
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {START_PATHS.map((path,i)=>(
                  <button key={i} onClick={path.action} style={{display:"flex",alignItems:"center",gap:16,padding:"18px 20px",borderRadius:16,border:"1px solid rgba(196,137,58,0.18)",background:"rgba(255,255,255,0.025)",cursor:"pointer",textAlign:"left",width:"100%",transition:"all 0.22s ease",outline:"none",position:"relative",overflow:"hidden"}}
                    onMouseEnter={e=>{e.currentTarget.style.background=path.accentLight;e.currentTarget.style.borderColor=path.accent;e.currentTarget.style.transform="translateX(4px)";}}
                    onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,0.025)";e.currentTarget.style.borderColor="rgba(196,137,58,0.18)";e.currentTarget.style.transform="translateX(0)";}}>
                    <div style={{width:44,height:44,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(255,255,255,0.04)",borderRadius:12,border:"1px solid rgba(255,255,255,0.06)",fontSize:"clamp(20px,5vw,24px)",flexShrink:0}}>{path.icon}</div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(14px,3.8vw,16px)",fontStyle:"italic",color:"rgba(247,242,234,0.9)",marginBottom:3}}>{path.label}</div>
                      <div style={{fontFamily:"Jost,sans-serif",fontSize:"clamp(11px,3vw,13px)",color:"rgba(247,242,234,0.4)",fontWeight:300,lineHeight:1.5}}>{path.sub}</div>
                    </div>
                    <div style={{flexShrink:0,fontSize:16,color:"rgba(196,137,58,0.4)",paddingRight:2}}>→</div>
                  </button>
                ))}
              </div>
      ...
      {(()=>{
        ...
        const [tActive, setTActive] = React.useState(0);
        const [tPaused, setTPaused] = React.useState(false);
        React.useEffect(()=>{
          if(tPaused) return;
          const timer = setInterval(()=>setTActive(a=>(a+1)%TESTIMONIALS_DATA.length),5000);
          return ()=>clearInterval(timer);
        },[tPaused]);
        const t = TESTIMONIALS_DATA[tActive];
        return (
          <section style={{background:"linear-gradient(180deg,#0E1610 0%,#0A0F0B 100%)",padding:"72px 20px 80px",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{maxWidth:900,margin:"0 auto",position:"relative"}}>
              <div style={{textAlign:"center",marginBottom:44}}>
                ...
                <p style={{fontFamily:"Jost,sans-serif",fontSize:"clamp(13px,3.2vw,15px)",color:"rgba(247,242,234,0.45)",fontWeight:300,lineHeight:1.7,maxWidth:380,margin:"0 auto"}}>Healing happens differently for everyone. Here is how it happened for them.</p>
              </div>
              <div style={{background:"linear-gradient(160deg,rgba(22,32,22,0.97),rgba(14,20,14,0.99))",border:"1px solid rgba(196,137,58,0.3)",borderRadius:24,padding:"36px 32px",position:"relative",overflow:"hidden",maxWidth:680,margin:"0 auto 32px",boxShadow:"0 16px 48px rgba(0,0,0,0.4)"}}>
                ...
                <div style={{width:42,height:42,borderRadius:12,background:`${t.accent}20`,border:`1px solid ${t.accent}40`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,marginBottom:20}}>{t.icon}</div>
                <blockquote style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(14px,3.5vw,16px)",fontStyle:"italic",color:"rgba(247,242,234,0.88)",lineHeight:1.9,margin:"0 0 24px"}}>{t.quote}</blockquote>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:20}}>
                <button onClick={()=>{setTPaused(true);setTActive(a=>(a-1+TESTIMONIALS_DATA.length)%TESTIMONIALS_DATA.length);}} style={{width:40,height:40,borderRadius:"50%",border:"1px solid rgba(196,137,58,0.3)",background:"transparent",color:"rgba(196,137,58,0.6)",fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} onMouseEnter={e=>{e.currentTarget.style.borderColor="#C4893A";e.currentTarget.style.color="#C4893A";}} onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(196,137,58,0.3)";e.currentTarget.style.color="rgba(196,137,58,0.6)";}}>←</button>
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  {TESTIMONIALS_DATA.map((_,i)=>(
                    <button key={i} onClick={()=>{setTPaused(true);setTActive(i);}} style={{width:tActive===i?20:6,height:6,borderRadius:3,background:tActive===i?"#C4893A":"rgba(196,137,58,0.25)",border:"none",cursor:"pointer",transition:"all 0.3s ease",padding:0}}/>
                  ))}
                </div>
                <button onClick={()=>{setTPaused(true);setTActive(a=>(a+1)%TESTIMONIALS_DATA.length);}} style={{width:40,height:40,borderRadius:"50%",border:"1px solid rgba(196,137,58,0.3)",background:"transparent",color:"rgba(196,137,58,0.6)",fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} onMouseEnter={e=>{e.currentTarget.style.borderColor="#C4893A";e.currentTarget.style.color="#C4893A";}} onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(196,137,58,0.3)";e.currentTarget.style.color="rgba(196,137,58,0.6)";}}>→</button>
              </div>
      ...
      <div style={{
        ...
        <img src="/vibe-shift-ring.jpg" alt="Vibe Shift Ring" style={{width:64,height:64,borderRadius:12,objectFit:"cover",border:"2px solid rgba(196,137,58,.35)",flexShrink:0,boxShadow:"0 4px 16px rgba(0,0,0,.5)"}}/>
        <div style={{flex:1,minWidth:200}}>
            ...
            <div className="modal-body" style={{padding:0}}>
              ...
              <div style={{padding:"24px 28px",background:"linear-gradient(160deg,#0E1A0E,#1A2E1A)",borderBottom:"1px solid rgba(196,137,58,.18)"}}>
                ...
                <div style={{background:"rgba(45,74,45,.5)",border:"1px solid rgba(196,137,58,.3)",borderRadius:14,padding:"16px 20px",textAlign:"center"}}>
                  ...
                  <div style={{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"}}>
                    <button
                      onClick={()=>window.open("https://2amcompanion.com","_blank","noopener,noreferrer")}
              ...
              <div style={{padding:"20px 28px",background:"#FAF7F0",borderRadius:"0 0 24px 24px"}}>
                ...
                <button className="btn-ghost" style={{fontSize:".72rem",padding:"10px 20px"}} onClick={()=>{ setIntentionOpen(false); setIntentionStep(0); setIntentionData({}); setIntentionResult(null); window.open("https://2amcompanion.com","_blank","noopener,noreferrer"); }}>
                  🌙 2AM Companion
                </button>
                </div>
              </div>
...
Thank you!`);
                            window.open("mailto:hello@chaiholistic.com?subject="+subject+"&body="+body,"_blank","noopener,noreferrer");
                          }}
        ...
        <div className="sec-in">
          ...
          <div style={{maxWidth:800,margin:"0 auto 3.5rem",textAlign:"center"}}>
            ...
            <div style={{display:"flex",justifyContent:"center",marginBottom:"2rem"}}>
              <div className="ring-spin-badge" onClick={()=>window.open("https://2amcompanion.com","_blank","noopener,noreferrer")}>
  ...
  return (
    ...
    <>
      ...
      {prayerOpen && (
        <>
          <div onClick={closePrayer} style={{position:"fixed",inset:0,zIndex:1800,background:"rgba(5,8,5,0.75)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",animation:"fadeIn .3s ease"}}/>
          <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:1801,height:"92vh",borderRadius:"24px 24px 0 0",overflow:"hidden",boxShadow:"0 -20px 80px rgba(0,0,0,0.6)",animation:"slideUp .35s cubic-bezier(.34,1.1,.64,1)",display:"flex",flexDirection:"column"}}>
            <div style={{background:"#0A0F0B",padding:"12px 20px 0",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,borderBottom:"1px solid rgba(196,137,58,0.12)"}}>
      ...
      {ingredientModal && (()=>{
        ...
        return (
          <>
            <div onClick={()=>setIngredientModal(null)} style={{position:"fixed",inset:0,zIndex:3000,background:"rgba(0,0,0,.75)",backdropFilter:"blur(8px)"}}/>
              ...
              <div style={{background:"linear-gradient(160deg,#FAF6EF,#F2EDE2)",border:"1px solid rgba(196,137,58,.25)",borderRadius:24,width:"100%",maxWidth:480,boxShadow:"0 32px 80px rgba(0,0,0,.4)",position:"relative",overflow:"hidden",maxHeight:"85vh",overflowY:"auto"}}>
                ...
                <div style={{padding:"18px 28px 24px"}}>
                  {ingredientModal.loading ? (
                    ...
                    <div style={{fontFamily:"Jost,sans-serif",fontSize:".82rem",color:"#5A4A3A",lineHeight:1.85,whiteSpace:"pre-wrap",fontWeight:300}}>{ingredientModal.info}</div>
                  )}
                </div>
                <div style={{padding:"0 28px 24px",display:"flex",gap:10}}>
                  <button onClick={()=>setIngredientModal(null)} style={{flex:1,background:"rgba(196,137,58,.12)",border:"1px solid rgba(196,137,58,.3)",color:"var(--bark)",borderRadius:14,padding:"11px",fontFamily:"Jost,sans-serif",fontSize:".68rem",letterSpacing:".1em",textTransform:"uppercase",cursor:"pointer",fontWeight:600}}>← Back to Tea Card</button>
                  <button onClick={()=>{setIngredientModal(null);nav("supplements");}} style={{flex:1,background:"linear-gradient(135deg,var(--bark),#3A2A18)",color:"white",border:"none",borderRadius:14,padding:"11px",fontFamily:"Jost,sans-serif",fontSize:".68rem",letterSpacing:".1em",textTransform:"uppercase",cursor:"pointer",fontWeight:600}}>View Supplements →</button>
                </div>
              </div>
            </div>
          </>
