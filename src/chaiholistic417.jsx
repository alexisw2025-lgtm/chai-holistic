...
class FinderErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.error) {
      return (
        <div style={{position:"fixed",inset:0,background:"rgba(28,26,23,.9)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:900,padding:20}}>
          <div style={{background:"white",borderRadius:16,padding:24,maxWidth:400,width:"100%"}}>
            <div style={{fontFamily:"serif",fontSize:"1.1rem",marginBottom:8}}>Something went wrong</div>
            <div style={{fontSize:".75rem",color:"red",marginBottom:16,wordBreak:"break-all"}}>{this.state.error.message}</div>
            <button onClick={()=>{this.setState({error:null});}} style={{background:"#275c3e",color:"white",border:"none",padding:"8px 20px",borderRadius:50,cursor:"pointer"}}>Close</button>
          </div>
        </div>
      );
    }
...
Thank you!`);
  ...
  return (
    <>
      ...
      {finderOpen && <FinderErrorBoundary><TeaFinderModal/></FinderErrorBoundary>}
