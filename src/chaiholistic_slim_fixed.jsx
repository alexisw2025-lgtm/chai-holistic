    ...
    <div>
      ...
      <div className="mq">
        <div className="mq-track">
          {[...Array(2)].map((_,p)=>["Morning Rituals","Evening Calm","Liver Cleanse","Kidney Flush","Vibe Shift Rings","Tea Finder","Ritual Builder","Cleanse Tracker","Herb Pairing Guide","Sip &amp; Heal · 40 Recipes"].map((t,i)=>(
            <div key={`${p}-${i}`} className="mq-item" style={{cursor:"pointer"}} onClick={()=>{const m={"Morning Rituals":()=>{nav("shop");setBlendFilter("Morning");},"Evening Calm":()=>{nav("shop");setBlendFilter("Evening");},"Liver Cleanse":()=>nav("shop"),"Kidney Flush":()=>nav("shop"),"Vibe Shift Rings":()=>nav("rings"),"Tea Finder":()=>setFinderOpen(true),"Ritual Builder":()=>setRitualOpen(true),"Cleanse Tracker":()=>setTrackerOpen(true),"Herb Pairing Guide":()=>nav("shop"),"Sip &amp; Heal · 40 Recipes":()=>nav("recipes")};const a=m[t];if(a)a();}}><span>✦</span>{t}</div>
          )))}
        </div>
      </div>
          ...
          <div style={{display:"flex",gap:14,overflowX:"auto",paddingBottom:10,scrollbarWidth:"none",msOverflowStyle:"none",WebkitOverflowScrolling:"touch"}}>
            {[
              ...
              return (
                <div
                  key={i}
                  onClick={()=>nav("tea-library",{blend:b.n})}
              ...
              <div key={b.id} className="pcard">
                ...
                <div className="pcard-body">
                  ...
                  <div className="pcard-foot">
                    ...
                    <button className="btn-add" onClick={()=>addToCart({...b,emoji:"🍵"})}>Add to Cart</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="ft-in">
          <div className="ft-grid">
            ...
            <div>
              ...
              {[["Tea Blends","shop"],["Cleansing Blends","shop"],["Individual Herbs","shop"],["Bundle & Save","shop"],["Vibe Shift Rings","rings"]].map(([l,p])=><span key={l} className="ft-lnk" onClick={()=>nav(p)}>{l}</span>)}
            </div>
            <div>
