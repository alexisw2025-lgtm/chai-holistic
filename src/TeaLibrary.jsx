...
function BlendModal({ blend, onClose, onPrev, onNext, current, total }) {
...
class ModalErrorBoundary extends React.Component {
  ...
  componentDidCatch(e, info) { console.error("Modal crash:", e, info); }
  render() {
    if (this.state.error) {
      return (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,padding:20}} onClick={()=>this.setState({error:null})}>
          <div style={{background:"white",borderRadius:16,padding:24,maxWidth:500,width:"100%"}}>
            <div style={{fontFamily:"serif",fontSize:"1.1rem",marginBottom:8,color:"red"}}>Error: {this.state.error.message}</div>
            <div style={{fontSize:".75rem",color:"#666",marginBottom:16,fontFamily:"monospace",whiteSpace:"pre-wrap"}}>{this.state.error.stack?.substring(0,300)}</div>
