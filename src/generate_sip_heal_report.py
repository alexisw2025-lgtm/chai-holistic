"""
generate_sip_report.py
Chai Holistic — Personalized Sip & Heal Report PDF Generator

Usage:
    python generate_sip_report.py --output /path/to/output.pdf
...
class BlendCard(Flowable):
    """Renders a single blend card for the Sip Report."""
...
def generate_prescription(profile: dict, output_path: str = None) -> bytes:
    """
    Generate a Sip & Heal Report PDF.
    ...
    first = name.split()[0]
    intro_text = (
        f"This Sip Report was created specifically for {first} based on a personalized "
    ...
    # ── YOUR BLENDS ────────────────────────────────────────────────────
    story.append(Paragraph("YOUR BLENDS", s_section))
    ...
    # Ritual box
    ritual_table = Table(
        [[Paragraph(ritual or "Brew your curated blend each day with intention. Hold the cup with both hands, breathe in the steam, and set one intention before your first sip.", s_ritual)]],
        colWidths=[usable_w]
    )
    ...
    story.append(Spacer(1, 8))
    disclaimer = (
        "These statements have not been evaluated by the FDA. This Sip Report is for "
